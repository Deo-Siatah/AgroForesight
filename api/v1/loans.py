...
def get_loan(loan_id: uuid.UUID, db: Session = Depends(get_db)) -> LoanRead:
    return LoanService(db).get_loan(loan_id)


# --- Status transitions ---------------------------------------------------

@router.patch("/{loan_id}/approve", response_model=LoanRead)
def approve_loan(loan_id: uuid.UUID, db: Session = Depends(get_db)) -> LoanRead:
    return LoanService(db).approve_loan(loan_id)


@router.patch("/{loan_id}/reject", response_model=LoanRead)
def reject_loan(loan_id: uuid.UUID, db: Session = Depends(get_db)) -> LoanRead:
    return LoanService(db).reject_loan(loan_id)


@router.patch("/{loan_id}/disburse", response_model=LoanRead)
def disburse_loan(loan_id: uuid.UUID, db: Session = Depends(get_db)) -> LoanRead:
    return LoanService(db).disburse_loan(loan_id)


@router.patch("/{loan_id}/activate", response_model=LoanRead)
def activate_loan(loan_id: uuid.UUID, db: Session = Depends(get_db)) -> LoanRead:
    return LoanService(db).activate_loan(loan_id)


@router.patch("/{loan_id}/repay", response_model=LoanRead)
def repay_loan(loan_id: uuid.UUID, db: Session = Depends(get_db)) -> LoanRead:
    return LoanService(db).repay_loan(loan_id)


@router.patch("/{loan_id}/default", response_model=LoanRead)
def default_loan(loan_id: uuid.UUID, db: Session = Depends(get_db)) -> LoanRead:
    return LoanService(db).default_loan(loan_id)
...
def get_risk(
    ...
    """
    return RiskService(db).calculate_risk(loan_id, season_id=season_id)
